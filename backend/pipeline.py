"""
pipeline.py — Layer 2 & 4: Pre/Post Processing Pipeline

KEY FIXES:
1. Tokens now use UUID format: ⟨a3f9b⟩ — model cannot transliterate these
2. Double bracket fix: Petitioner [Petitioner [x]] → Petitioner [x]
3. Transliteration map expanded to catch more variants
"""
...
class PreProcessor:
    def process(self, text: str) -> Tuple[str, Dict[str, str]]:
        ...
        def token(original: str) -> str:
            # Use short random hex ID wrapped in Unicode angle brackets
            # ⟨a3f9b⟩ — model treats this as a code/symbol, never translates it
